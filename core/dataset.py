...
def get_ve8(image_path,audio_path,opt, subset, transforms,index=None):
    #print("index",index)
...
#新增了path在输入
def get_validation_set(image_path,audio_path,opt, spatial_transform, temporal_transform, target_transform,index=None):
    #print("index:",index)
    if opt.dataset == 've8':
        transforms = [spatial_transform, temporal_transform, target_transform]
        return get_ve8(image_path,audio_path, opt, 'validation', transforms,index)
    else:
        raise Exception
